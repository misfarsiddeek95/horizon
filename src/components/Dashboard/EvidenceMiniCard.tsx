...
export default function EvidenceMiniCard({
  ...
  return (
    <article
      ...
      onClick={onClick}
    >
      <h5 className="text-xs font-extrabold text-[#344257] leading-[1.3]">
        {title}
      </h5>
      <p className="text-[11px] text-[#4A586B] leading-[1.45]">{insight}</p>
      <div ref={sparklineRef} className="h-16 w-full" />
      <div className="flex items-center justify-between gap-2">
        <span className="text-[11px] font-extrabold text-brand-main">
          View chart &rarr;
        </span>
        {onSourcesClick && (
          <button
            ...
            className="text-[10px] font-extrabold border border-[#D2DDE6] bg-white text-[#071D43] rounded-lg px-2 py-1 hover:bg-[#EAF5F6] hover:border-[#9FCFD2] hover:text-[#116D72] transition-colors whitespace-nowrap"
          >
            Sources
          </button>
        )}
