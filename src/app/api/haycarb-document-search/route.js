...
//
// Proxies document-search requests to the Haycarb API so the API key never reaches
...
const MAX_REQUESTS = 25;          // per IP per minute — cheaper than chat
  ...
  try {
    ...
    if (!limit.allowed) {
      return Response.json(
        {
          error: 'rate_limit',
          message: `You've made a lot of searches. Please wait ${limit.retryAfter} seconds and try again.`,
    ...
    const res = await fetch(`${apiBase}/api/document-search`, {
      method: 'POST',
      headers: {
    ...
    console.error('Haycarb document-search proxy error', error);
